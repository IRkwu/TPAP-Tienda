...
def GuardarExcel(articulo):
    ...
    df = pd.DataFrame({
        'Nombre': [articulo.get_nombre() for articulo in ListaArticulos],
        'Mascota': [articulo.get_mascota() for articulo in ListaArticulos],
        'ID': [articulo.get_id() for articulo in ListaArticulos],
        'Marca': [articulo.get_marca() for articulo in ListaArticulos],
        'Precio por unidad': [articulo.get_precio_por_unidad() for articulo in ListaArticulos],
        'Stock': [articulo.get_stock() for articulo in ListaArticulos],
        'Descripcion': [articulo.get_descripcion() for articulo in ListaArticulos],
        'Categoria': [articulo.get_categoria() for articulo in ListaArticulos],
        'Precio por lote': [articulo.get_precio_por_lote() for articulo in ListaArticulos],
        'Limite critico': [articulo.get_limite_critico() for articulo in ListaArticulos]
    })
    ...
    writer._save()
GuardarExcel(ListaArticulos)

print(ListaArticulos[0].get_nombre())
print(ListaArticulos[1].get_nombre())