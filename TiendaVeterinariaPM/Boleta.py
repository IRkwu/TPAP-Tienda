...
class Boleta:
    ...
    def GuardarCSV_Boletas(self, historial_boletas):
        with open('Archivos de Datos\HistorialBoletas.csv', mode='w', newline='') as file:
            writer = csv.writer(file, delimiter=',',
                                quotechar='"', quoting=csv.QUOTE_MINIMAL)
            writer.writerow(['id_transaccion', 'nombre_vendedor',
                            'nombre_cliente', 'fecha', 'articulos', 'total', 'costo_envio'])
            for boleta in historial_boletas:
                writer.writerow([boleta.get_id_transaccion(), boleta.get_nombre_vendedor(), boleta.get_nombre_cliente(
                ), boleta.get_fecha(), boleta.get_articulos(), boleta.get_total(), boleta.get_costo_envio()])

    # Funcion para cargar los envios desde un archivo csv
    def CargarCSV_Boletas(self, ruta_archivo):
        historial_boletas = []
        with open(ruta_archivo, mode='r') as file:
            ...
            for row in reader:
                ...
                boleta = Boleta(id_transaccion, nombre_vendedor,
                                nombre_cliente, fecha, articulos, total, costo_envio)
                historial_boletas.append(boleta)
        return historial_boletas

    # Funcion para agregar las boletas en un archivo csv
    def AgregarBoleta(self, id_transaccion, nombre_vendedor, nombre_cliente, fecha, articulos, total, costo_envio):
        boleta = Boleta(id_transaccion, nombre_vendedor,
                        nombre_cliente, fecha, articulos, total, costo_envio)
        HistorialBoletas.append(boleta)
        Boleta.GuardarCSV_Boletas(None, HistorialBoletas)
...
HistorialBoletas = Boleta.CargarCSV_Boletas(None,
                                            'Archivos de Datos\HistorialBoletas.csv')
